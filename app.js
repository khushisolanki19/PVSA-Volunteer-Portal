...
const state = {
  ...
  projects: [
    ...
    { id:"p2", title:"Habitat for Humanity", type:"Community Building", date:"2026-08-16", signupCloseDate:"2026-08-15", waitlistEnabled:true, time:"8:30 AM–11:30 AM", location:"Fremont Build Site", slots:8, status:"Open", hours:3, leadIds:["a1"], description:"Support a Habitat for Humanity build and help prepare materials for volunteers.", signups:["y2","a1"], waitlist:[], checkedIn:[] },
    { id:"p3", title:"Community Donation Drive", type:"Donation Drive", date:"2026-09-05", signupDate:"2026-08-20", signupCloseDate:"2026-09-04", waitlistEnabled:true, time:"10:00 AM–2:00 PM", location:"JCNC Main Hall", slots:20, status:"Upcoming", hours:4, leadIds:["a1"], description:"Collect, sort, and prepare donated essentials for neighbors in need.", signups:[], waitlist:[], checkedIn:[] },
    { id:"p4", title:"Second Harvest Volunteer Shift", type:"Hunger Relief", date:"2026-07-12", time:"1:00 PM–4:00 PM", location:"Second Harvest Warehouse", slots:6, status:"Completed", hours:3, leadIds:["a1"], description:"Pack food boxes and prepare pantry orders for distribution.", signups:["y1","a1"], waitlist:[], checkedIn:["y1","a1"], attendance:{y1:{lateMinutes:20,note:"Parent let us know Riana arrived late because of a school event."},a1:{lateMinutes:0,note:""}} }
  ],
  logs: [
...
function renderAll() { renderHome(); renderProjects(); renderHours(); renderCommunity(); renderManage(); }
...
function renderHome() {
  ...
  const joined = state.projects.filter(p => (p.signups.includes(u.id) || projectLeadIds(p).includes(u.id)) && p.status !== "Completed");
  $("#home-view").innerHTML = `
...
function projectCard(p) {
  const signed = p.signups.includes(state.userId), wait = p.waitlist.includes(state.userId), isLead=projectLeadIds(p).includes(state.userId), left = Math.max(0,p.slots-p.signups.length);
  return `<article class="project-card" onclick="openProject('${p.id}')"><div class="date-tile"><b>${new Date(p.date+'T12:00:00').toLocaleString('en',{month:'short'})}</b><strong>${new Date(p.date+'T12:00:00').getDate()}</strong></div><div class="project-info"><div class="badge-row"><span class="badge ${p.status.toLowerCase()}">${p.status}</span>${isLead?'<span class="badge lead">Parent Lead</span>':''}<span>${escapeHtml(p.type)}</span></div><h3>${escapeHtml(p.title)}</h3><p>${p.time} · ${escapeHtml(p.location)}</p><div class="slot-row"><span>${isLead?'★ You’re a Parent Lead':signed?'✓ You’re signed up':wait?'Waitlist':p.status==='Open'?`${left} of ${p.slots} spots left`:`${p.signups.length} volunteers`}</span><span>›</span></div></div></article>`;
}

function openProject(id) {
  const p=project(id), u=currentUser();
  const isLead = projectLeadIds(p).includes(u.id), isEventDay=p.date===new Date().toISOString().slice(0,10);
  const canAttend = u.role==='Coordinator' || (isLead && isEventDay);
  const eligible = linkedSignupAccounts(u);
  const leadNames=projectLeadIds(p).map(id=>user(id)?.name).filter(Boolean);
  const closed=isSignupClosed(p);
  openDialog(`<p class="eyebrow">${escapeHtml(p.type)}</p><h2>${escapeHtml(p.title)}</h2><p class="dialog-lead">${escapeHtml(p.description)}</p><dl class="details"><div><dt>Date & time</dt><dd>${fmtDate(p.date)}<br>${p.time}</dd></div><div><dt>Signup closes</dt><dd>${p.signupCloseDate?fmtDate(p.signupCloseDate):'Coordinator controlled'}${closed?'<br><b>Closed</b>':''}</dd></div><div><dt>Location</dt><dd>${escapeHtml(p.location)}</dd></div><div><dt>Parent Leads</dt><dd>${escapeHtml(leadNames.join(', ') || 'To be assigned')}</dd></div><div><dt>Service credit</dt><dd>${p.hours} hours</dd></div><div><dt>Waitlist</dt><dd>${p.waitlistEnabled?'Enabled':'Not offered'}</dd></div></dl><div class="capacity"><span><b>${p.signups.length}/${p.slots}</b> spots filled${p.waitlist.length?` · ${p.waitlist.length} waitlisted`:''}</span><div class="progress"><i style="width:${Math.min(100,p.signups.length/p.slots*100)}%"></i></div></div>${p.status==='Open'&&!closed ? `<div class="signup-family"><b>Who are you signing up?</b>${eligible.map(person=>signupControl(p,person)).join('')}</div>` : closed?`<div class="signup-closed"><b>Signup is closed</b><p>Only a Coordinator can change the roster now.</p></div>`:`<button class="secondary wide" disabled>${p.status==='Upcoming'?'Signup opens '+fmtDate(p.signupDate):p.status}</button>`}${canAttend ? `<button class="secondary wide" onclick="attendance('${p.id}')">Take attendance</button>`:isLead?`<p class="attendance-locked">Attendance opens for Parent Leads on ${fmtDate(p.date)}.</p>`:''}`);
}

function projectLeadIds(p){return p.leadIds||[p.leadId].filter(Boolean);}
function isSignupClosed(p){return Boolean(p.manualClosed||(p.signupCloseDate&&new Date().toISOString().slice(0,10)>=p.signupCloseDate));}

function linkedSignupAccounts(u){const ids=[u.id,...(u.youthIds||[]),...(u.adultId?[u.adultId]:[])];return [...new Set(ids)].map(user).filter(Boolean);}
function signupControl(p,person){const signed=p.signups.includes(person.id),wait=p.waitlist.includes(person.id),full=p.signups.length>=p.slots;return `<div><span><b>${escapeHtml(person.name)}</b><small>${person.id===state.userId?'Your account':person.role==='Youth'?'Linked child':'Linked parent'}</small></span><button class="${signed||wait?'secondary':'primary'}" ${full&&!p.waitlistEnabled&&!signed?'disabled':''} onclick="toggleSignup('${p.id}','${person.id}')">${signed?'Cancel':wait?'Leave waitlist':full&&p.waitlistEnabled?'Join waitlist':full?'Full':'Sign up'}</button></div>`;}
...
function toggleSignup(id,uid=state.userId) {
  ...
  const p=project(id);
  if(isSignupClosed(p))return toast('Signup is closed. Only a Coordinator can change the roster.');
  ...
  else if (p.signups.length>=p.slots && p.waitlistEnabled) { p.waitlist.push(uid); toast("You’re on the waitlist."); }
  else if (p.signups.length>=p.slots) { return toast("This event is full and does not offer a waitlist."); }
  else { p.signups.push(uid); toast("You’re signed up!"); }
  closeDialog(); renderAll();
}

function calendarSection(){const u=currentUser();const events=u.role==='Coordinator'?state.projects:state.projects.filter(p=>p.signups.includes(u.id)||projectLeadIds(p).includes(u.id));return `<section><div class="section-title calendar-title"><div><p class="eyebrow">August 2026</p><h2>${u.role==='Coordinator'?'All-events calendar':'My calendar'}</h2></div><span>${events.length} events</span></div><div class="calendar"><div class="calendar-week">${['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map(d=>`<b>${d}</b>`).join('')}</div><div class="calendar-grid">${calendarDays(2026,7,events)}</div></div></section>`;}
...
function approvedHoursFor(member){const poolIds=member.role==='Adult'&&member.familyId?state.users.filter(u=>u.role==='Adult'&&u.familyId===member.familyId).map(u=>u.id):[member.id];return state.logs.filter(l=>poolIds.includes(l.userId)&&l.status==='Approved').reduce((n,l)=>n+l.hours,poolIds.reduce((n,id)=>n+(user(id)?.hours||0),0));}
function renderCommunity(){const people=state.users.filter(u=>u.role==='Youth'||u.role==='Adult').sort((a,b)=>a.name.split(' ')[0].localeCompare(b.name.split(' ')[0]));$('#community-view').innerHTML=`<div class="page-title"><div><p class="eyebrow">Program progress</p><h1>Everyone’s hours</h1><p class="dialog-lead">Youth and Parent volunteer progress, ordered by first name.</p></div></div><div class="leaderboard">${people.map((person,index)=>{const hours=approvedHoursFor(person),pct=Math.min(100,hours/person.goal*100);return `<article class="leader-row"><span class="rank">${index+1}</span><span class="avatar small">${person.name.split(' ').map(x=>x[0]).join('')}</span><div><b>${escapeHtml(person.name)}</b><small>${person.role==='Adult'?'Parent volunteer · shared family goal':person.goal===100?'Youth · 100-hour goal':'Youth · 250-hour goal'}</small><div class="progress"><i style="width:${pct}%"></i></div></div><strong>${hours} <small>/ ${person.goal}h</small></strong></article>`}).join('')}</div>`;}
...
function renderManage() {
  ...
  $("#manage-view").innerHTML=`<div class="page-title"><div><p class="eyebrow">Coordinator</p><h1>Program management</h1></div></div><div class="admin-grid"><button onclick="projectForm()"><span>＋</span><b>Create project</b><small>Dates, slots and Parent Leads</small></button><button onclick="showView('hours')"><span>✓</span><b>Hour approvals</b><small>${pending} waiting</small></button><button onclick="memberDirectory()"><span>♙</span><b>Members</b><small>${state.users.length} enrolled</small></button><button onclick="exportReport()"><span>⇩</span><b>Export report</b><small>PVSA-ready CSV</small></button></div><section><div class="section-title"><h2>Project overview</h2></div>${state.projects.map(p=>`<article class="manage-row"><span class="badge ${isSignupClosed(p)?'completed':p.status.toLowerCase()}">${isSignupClosed(p)?'Signup closed':p.status}</span><div><b>${escapeHtml(p.title)}</b><small>${fmtDate(p.date)} · ${p.signups.length}/${p.slots} signed up · ${p.waitlist.length} waitlisted</small></div><div class="row-actions"><button onclick="toggleProjectSignup('${p.id}')">${isSignupClosed(p)?'Reopen':'Close'}</button><button onclick="manageRoster('${p.id}')">Signups</button><button onclick="assignLeadsForm('${p.id}')">Leads</button><button onclick="openProject('${p.id}')">View</button></div></article>`).join('')}</section>`;
}

function leadCheckboxes(selected=[]){return state.users.filter(u=>u.role==='Adult').map(adult=>`<label class="check-row"><input type="checkbox" name="leadIds" value="${adult.id}" ${selected.includes(adult.id)?'checked':''}><span><b>${escapeHtml(adult.name)}</b><small>Parent volunteer</small></span></label>`).join('')||'<p>No Adult members are available yet.</p>';}
function projectForm(){openDialog(`<p class="eyebrow">Coordinator</p><h2>Create a project</h2><form onsubmit="createProject(event)"><label>Project name<input name="title" required></label><div class="form-row"><label>Event date<input name="date" type="date" required></label><label>Signup closing date<input name="signupCloseDate" type="date" required></label></div><div class="form-row"><label>Volunteer slots<input name="slots" type="number" min="1" value="10" required></label><label class="check-row"><input name="waitlistEnabled" type="checkbox" checked><span><b>Offer waitlist</b><small>Auto-promote when a spot opens</small></span></label></div><label>Location<input name="location" required></label><label>Description<textarea name="description" required></textarea></label><fieldset><legend>Parent Leads <small>(choose one or more now, or assign later)</small></legend>${leadCheckboxes()}</fieldset><button class="primary wide" type="submit">Create project</button></form>`)}
function createProject(e){e.preventDefault();const d=new FormData(e.target),date=d.get('date'),closeDate=d.get('signupCloseDate');if(closeDate>date)return toast('Signup must close on or before the event date.');state.projects.unshift({id:'p'+Date.now(),title:d.get('title'),type:'Community',date,signupCloseDate:closeDate,waitlistEnabled:d.get('waitlistEnabled')==='on',time:'Time TBD',location:d.get('location'),slots:Number(d.get('slots')),status:'Upcoming',hours:0,leadIds:d.getAll('leadIds'),description:d.get('description'),signups:[],waitlist:[],checkedIn:[]});closeDialog();renderAll();toast('Project created with its signup deadline.');}
function assignLeadsForm(id){const p=project(id);openDialog(`<p class="eyebrow">Coordinator</p><h2>Assign Parent Leads</h2><p class="dialog-lead">${escapeHtml(p.title)}</p><form onsubmit="saveLeads(event,'${p.id}')"><fieldset><legend>Parent Leads</legend>${leadCheckboxes(projectLeadIds(p))}</fieldset><button class="primary wide" type="submit">Save Parent Leads</button></form>`)}
function saveLeads(e,id){e.preventDefault();const p=project(id),d=new FormData(e.target);p.leadIds=d.getAll('leadIds');fetch(APPS_SCRIPT_URL,{method:'POST',mode:'no-cors',body:JSON.stringify({action:'assignLeads',projectId:id,leadIds:p.leadIds})}).catch(()=>{});closeDialog();renderAll();toast('Parent Leads updated. Their calendars now include this event.');}
function toggleProjectSignup(id){const p=project(id);p.manualClosed=!isSignupClosed(p);if(!p.manualClosed&&p.signupCloseDate&&new Date().toISOString().slice(0,10)>=p.signupCloseDate)p.signupCloseDate=p.date;fetch(APPS_SCRIPT_URL,{method:'POST',mode:'no-cors',body:JSON.stringify({action:'setSignupClosed',projectId:id,closed:p.manualClosed})}).catch(()=>{});renderAll();toast(p.manualClosed?'Signup closed. Only Coordinators can edit the roster.':'Signup reopened.');}
function manageRoster(id){const p=project(id),members=state.users.filter(u=>u.role==='Youth'||u.role==='Adult');openDialog(`<p class="eyebrow">Coordinator roster override</p><h2>${escapeHtml(p.title)}</h2><p class="dialog-lead">Changes here are allowed even after signup closes.</p><form onsubmit="saveRoster(event,'${p.id}')"><fieldset><legend>Confirmed volunteers (${p.signups.length}/${p.slots})</legend>${members.map(m=>`<label class="check-row"><input type="checkbox" name="signups" value="${m.id}" ${p.signups.includes(m.id)?'checked':''}><span><b>${escapeHtml(m.name)}</b><small>${m.role}</small></span></label>`).join('')}</fieldset>${p.waitlistEnabled?`<fieldset><legend>Waitlist</legend>${members.map(m=>`<label class="check-row"><input type="checkbox" name="waitlist" value="${m.id}" ${p.waitlist.includes(m.id)?'checked':''}><span><b>${escapeHtml(m.name)}</b><small>${m.role}</small></span></label>`).join('')}</fieldset>`:''}<button class="primary wide" type="submit">Save roster</button></form>`)}
function saveRoster(e,id){e.preventDefault();const p=project(id),d=new FormData(e.target),signups=[...new Set(d.getAll('signups'))],waitlist=[...new Set(d.getAll('waitlist'))].filter(uid=>!signups.includes(uid));if(signups.length>p.slots)return toast(`Only ${p.slots} confirmed spots are available.`);p.signups=signups;p.waitlist=waitlist;fetch(APPS_SCRIPT_URL,{method:'POST',mode:'no-cors',body:JSON.stringify({action:'saveRoster',projectId:id,signups,waitlist})}).catch(()=>{});closeDialog();renderAll();toast('Event roster updated.');}
function memberDirectory(){openDialog(`<p class="eyebrow">Private · Coordinators only</p><h2>Member directory</h2><p class="dialog-lead">Contact details below are visible only to Coordinators.</p><button class="primary wide" onclick="addMemberForm()">+ Add a new member</button><div class="member-list">${state.users.map(u=>`<div class="contact-record"><span class="avatar small">${u.name.split(' ').map(x=>x[0]).join('')}</span><span><b>${u.name}</b><small>${u.role}${u.goal?` · ${u.goal}h goal`:''}</small><small><b>Sign-in:</b> ${u.email}</small><small><b>Contact email:</b> ${u.contactEmail||'N/A'} · <b>Phone:</b> ${u.phone||'N/A'}</small></span></div>`).join('')}</div>`)}
function addMemberForm(){const adults=state.users.filter(u=>u.role==='Adult');openDialog(`<p class="eyebrow">Coordinator enrollment</p><h2>Add a member</h2><form onsubmit="addMember(event)"><label>Full name<input name="name" required autocomplete="name"></label><label>Account type<select name="role" required onchange="toggleYouthFields(this.value)"><option value="Youth">Youth volunteer</option><option value="Adult">Adult / Parent volunteer</option></select></label><label>Sign-in email<input name="email" type="email" required autocomplete="email"><small>Required so the member can sign in.</small></label><div class="form-row"><label>Contact email<input name="contactEmail" placeholder="Email or N/A"></label><label>Phone number<input name="phone" type="tel" placeholder="Phone or N/A"></label></div><label>Temporary private access code<input name="accessCode" minlength="8" required placeholder="At least 8 characters"><small>Share this privately. The member uses it with their email.</small></label><div id="youth-enrollment"><label>Youth service goal<select name="goal"><option value="100">100 hours</option><option value="250">250 hours</option></select></label><div class="parent-required"><b>Parent volunteer required</b><p>Every Youth must be linked to an Adult who also volunteers.</p><label>Parent option<select name="parentMode" onchange="toggleParentMode(this.value)"><option value="existing">Choose an existing Parent</option><option value="new">Add a new Parent now</option></select></label><div id="existing-parent-fields"><label>Existing Parent<select name="adultId"><option value="">Select a Parent</option>${adults.map(a=>`<option value="${a.id}">${escapeHtml(a.name)}</option>`).join('')}</select></label></div><div id="new-parent-fields" class="hidden"><label>Parent full name<input name="parentName" autocomplete="name"></label><label>Parent sign-in email<input name="parentEmail" type="email" autocomplete="email"></label><div class="form-row"><label>Parent contact email<input name="parentContactEmail" placeholder="Email or N/A"></label><label>Parent phone<input name="parentPhone" type="tel" placeholder="Phone or N/A"></label></div><label>Parent access code<input name="parentAccessCode" minlength="8" placeholder="At least 8 characters"></label></div></div></div><button class="primary wide" type="submit">Add member and send to Sheet</button></form>`)}
function toggleYouthFields(role){$('#youth-enrollment')?.classList.toggle('hidden',role!=='Youth');}
function toggleParentMode(mode){$('#existing-parent-fields')?.classList.toggle('hidden',mode!=='existing');$('#new-parent-fields')?.classList.toggle('hidden',mode!=='new');}
function addMember(e){e.preventDefault();const d=new FormData(e.target),email=d.get('email').trim().toLowerCase(),role=d.get('role');if(state.users.some(u=>u.email.toLowerCase()===email))return toast('A member already uses that email.');let adultId='',newParent=null;if(role==='Youth'&&d.get('parentMode')==='existing'){adultId=d.get('adultId');if(!adultId)return toast('Select the Youth’s Parent volunteer.');}if(role==='Youth'&&d.get('parentMode')==='new'){const parentName=d.get('parentName').trim(),parentEmail=d.get('parentEmail').trim().toLowerCase(),parentCode=d.get('parentAccessCode');if(!parentName||!parentEmail||!parentCode||parentCode.length<8)return toast('Complete all required new Parent fields.');if(state.users.some(u=>u.email.toLowerCase()===parentEmail)||parentEmail===email)return toast('That Parent email is already being used.');adultId='a'+Date.now();newParent={id:adultId,name:parentName,role:'Adult',email:parentEmail,contactEmail:d.get('parentContactEmail').trim()||'N/A',phone:d.get('parentPhone').trim()||'N/A',accessCode:parentCode,hours:0,goal:52,youthIds:[],familyId:`family-${adultId}`};state.users.push(newParent);}const id=(role==='Youth'?'y':'a')+(Date.now()+1),member={id,name:d.get('name').trim(),role,email,contactEmail:d.get('contactEmail').trim()||'N/A',phone:d.get('phone').trim()||'N/A',accessCode:d.get('accessCode'),hours:0,goal:role==='Adult'?52:Number(d.get('goal'))};if(adultId){member.adultId=adultId;const adult=user(adultId);adult.youthIds=[...new Set([...(adult.youthIds||[]),id])];member.familyId=adult.familyId||`family-${adult.id}`;adult.familyId=member.familyId;}if(role==='Adult'){member.youthIds=[];member.familyId=`family-${id}`;}state.users.push(member);fetch(APPS_SCRIPT_URL,{method:'POST',mode:'no-cors',body:JSON.stringify({action:'addMember',member,parent:newParent})}).catch(()=>{});closeDialog();renderAll();toast(`${member.name}${newParent?' and '+newParent.name:''} added successfully.`);}
